    ...
    while True:
        if status == 'player turn':
            ...
            for i in range(len(players)):
                ...
                print("")
                action = input(
                    f"{players[i]}가 대기중입니다. 어떻게 하시겠습니까? [ 1) 일반공격 2) 스킬:{players[ } 3) 포션 사용 4) 도망친다 ] : ")
                if action.isdigit() == False:
                    ...
                    if action == '1':
                        ...
                        if target.isdigit() == False:
                            ...
                            # monsters[int(target) - 1] = monsters[int(target) - 1]
                            check_ = players[i].normal_attack(
                                monsters[int(target) - 1])
                            if check_ == True:
                                earned_exp += int(monsters[int(target) - 1].level * 2 * random.randint(
                                    1, 2) + monsters[int(target) - 1].max_HP * 1.2 * random.randint(1, 2))
                                monsters[int(target) - 1] = '기절'
