...
class Object():
    ...
    def __init__(self, name, level, HP, MP, attack, defense, speed, skill=None, skill_cost=None) -> None:
        self.name = name
        self.level = level
        self.max_HP = HP
        self.HP = HP
...
class Character(Object):
    def __init__(self, name, level, HP, MP, attack, defense, speed, skill, skill_cost=None, eq='', exp=0, max_exp=100) -> None:
        super().__init__(name, level, HP, MP, attack, defense, speed, skill, skill_cost)
        self.exp = exp
    ...
    def dealing_Skill(self, monster):
        self.MP = max(self.MP - self.skill_cost, 0)
        if self.MP == 0:
...
#   캐릭터 전투시작 & 디폴트 스테이터스 창
    def status(self):
        return f"{self} LV. {self.level} || HP: ({self.HP} / {self.max_HP}) || MP: ({self.MP} / {self.max_MP})"
